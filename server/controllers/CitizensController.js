...
import { sequelize,Citizen} from '../models';
...
export default class CitizensController {
   ...
   static async create(req, res)  {
      ...
      try{
        ...
        const citizen = await Citizen.create({name, email,role});

        return res.json({message:'Citizen created successful', citizen})
      }
      catch(err){
        console.log(err)
    ...
    static async getAllCitizens(req, res)  {
      ...
      try{
        ...
        const citizen = await Citizen.findAll({name, email,role});

        return res.json({message:'list of all citizens', citizen})
      }
      catch(err){
        console.log(err)
    ...
    static async getOneCitizen(req, res)  {
      const uuid = req.params.uuid;
      try{

        const citizen = await Citizen.findOne({wwhere:{uuid}})
